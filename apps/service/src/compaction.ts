...
const PATH_MARKERS = ["bookDir=", "storyDir=", "chapterFile=", "authorBrief=", "[路径提醒]", "books/", "story/", "chapters/"];
...
export function extractStructuredSummary(
  ...
  for (const msg of middleMessages) {
    ...
    // Extract path hints (from both roles)
    const lines = content.split("\n");
    for (const line of lines) {
      if (PATH_MARKERS.some((m) => line.includes(m)) && line.trim().length > 6) {
